...
export async function getCatalog(){
  ...
  for(const chunk of chunks||[]){const value=map[chunk.khassida_id]||{};value.verses=Math.max(value.verses||0,chunk.verse_end||0);value.pages=Math.max(value.pages||0,chunk.page_number||0);map[chunk.khassida_id]=value}
  for(const item of media||[]){if(item.kind==="audio")map[item.khassida_id]={...(map[item.khassida_id]||{}),hasAudio:true}}
